...
let FieldBackground = LinearGradient(colors: [PaleYellow, PeachOrange], startPoint: .topLeading, endPoint: .bottomTrailing)

var AngGradietn: some View {
    AngularGradient(gradient: Gradient(colors: [ customRedColor, customMagentaColor, customCyanColor, customYellowColor, customRedColor ]), center: .center, angle: .degrees(120)).ignoresSafeArea()
}

var LinGradient: some View {
    LinearGradient(gradient: Gradient(colors: [Color.white.opacity(0), Color.white.opacity(1)]), startPoint: .bottom, endPoint: .top).ignoresSafeArea()
}